...
@click.option(
    "--search_value", prompt="Enter search value ", default="", help="searchOption."
)
...
def displaySearchResult(searchOptions, targetData):
    ...
    print(
        "\n\n ** Search Results for Search of {0} with SearchTerm = {1} and SearchValue = {2} **\n".format(
            optionDict["searchOption"],
            optionDict["searchTerm"],
            optionDict["searchValue"],
        )
    )
    ...
    elif searchOptions["searchOption"] in "Users":
        for result in targetData:
            print("\nUser Result: \n")
            pprint(result["users"])
    ...
    elif searchOptions["searchOption"] in "Tickets":
        for result in targetData:
            print("\nTicket Result: \n")
            pprint(result["tickets"])
